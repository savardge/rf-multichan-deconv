...
for ii=1:nipbuniq
    ...
    if ~isempty(ind)
        ...
        [urft0(ipb2,:),psumf1,beta]=simdecf(pft(ind,1:nfft/2+1),sft(ind,1:nfft/2+1),-5);
        [uzft0(ipb2,:),psumf2,beta]=simdecf(pft(ind,1:nfft/2+1),tft(ind,1:nfft/2+1),-5);
        ...
        % amplitude.
        dump=real(ifft([psumf1,fliplr(conj(psumf1(2:nfft/2)))],nfft));
        pmax_r(ipb2)=max(dump);
        dump=real(ifft([psumf2,fliplr(conj(psumf2(2:nfft/2)))],nfft));
        pmax_t(ipb2)=max(dump);
%         resfunc(ipb2,:)=dump;        
