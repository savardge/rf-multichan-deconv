...
% vertical
tmp = dir(fullfile(fdir,'OK',[net,'.',sta,'*HZ.*SAC']));
zfiles={tmp.name}';
% radial
tmp = dir(fullfile(fdir,'OK',[net,'.',sta,'*HR.*SAC']));
rfiles={tmp.name}';
% transverse
tmp = dir(fullfile(fdir,'OK',[net,'.',sta,'*HT.*SAC']));
tfiles={tmp.name}';

% Get events above SNR
snr = getsnr(fdir,zfiles,dtap,ndt0,ndec);
igood = find(snr>snrthresh);
zfiles=zfiles(igood);
rfiles=rfiles(igood);
tfiles=tfiles(igood);
...
snrall=zeros(nfiles,1); % signal-to-noise ratios
...
for ifile=1:nfiles
   ...
   % Retrieve vertical component and header params.  
    ...
    if (evdp(ifile) > 100 && evdp(ifile) <= 250 && head.T7>0) || isnan(head.T9)% then use "t7"
...
%     tcomp=detrendp(tcomp, 2);
    ...
    %% filter
%     zcomp=bpfilt(zcomp,ndt0,0.05,8);
%     scomp=bpfilt(rcomp,ndt0,0.05,8);
%     tcomp=bpfilt(tcomp,ndt0,0.05,8);
...
%     scomp=taper(scomp,dtap,ndt,dtap,(length(scomp)-1)*ndt-dtap); %GS
    ...
    %% plot
%     figure(100);
%     title(sprintf("SNR = %",snrall(ifile)))
...
%     subplot(3,1,1); plot(fp,ppp); xlim([0 1])
%     subplot(3,1,2); plot(fs,pps); xlim([0 1])
%     subplot(3,1,3); plot(ft,ppt); xlim([0 1])
%     
%     figure(102)
