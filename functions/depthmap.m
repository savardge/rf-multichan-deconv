...
fid=fopen(['../../velocity_model/SL2013NA_1D_0.25D/',sta,'.mod'],'r');
...
figure(13);clf
subplot(2,1,2);
zsection_pad(PRFrmat,z,-1,sbaz(isort),vbaz,nid(isort))
title(['Radial component, filtered between ',num2str(lp1),' - ',num2str(lp),' Hz'])
...
figure(15);clf
subplot(2,1,2)
zsection_pad(PRFtmat,z,-1,sbaz(isort),vbaz,nid(isort))
title(['Transverse component, filtered between ',num2str(lp1),' - ',num2str(lp),' Hz'])
grid on;box on;set(gca,'layer','top')
% set(gca,'Position',[0.1300    0.1100    0.7750    0.8150])
if savefig==1
