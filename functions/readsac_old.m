% function [t,a,p]=readsac_old(sacfile,npts,'many') 
% 
...
% Written by Greenfield&Battenhouse 
 ...
 function [t,a,p]=readsac_old(sacfile,varargin) 
...
% Need to specify bigendian to work on PGC/COR data sets.
 sacfid=fopen(sacfile,'r','b'); 
 ...
 for j=1:105, if p(j)==-12345 p(j)=nan; end, end; 
  npts=p(80); a=zeros(1,npts); t=zeros(1,npts); 
  t=p(6):p(1):p(6)+(npts-1)*p(1); 
